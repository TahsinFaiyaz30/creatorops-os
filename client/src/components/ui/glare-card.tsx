...
export const GlareCard = ({
  children,
  className,
  // Upstream hardcodes `[aspect-ratio:17/21] w-[320px]` on the outer wrapper and
  // only forwards `className` to the inner content div, so the card cannot be
  // sized or made responsive from outside. Exposed as a prop; the default string
  // is the original, so omitting it renders exactly as upstream does.
  ...
  const backgroundStyle = {
    ...
    "--rainbow":
      "repeating-linear-gradient( 0deg,rgb(255,119,115) calc(var(--step) * 1),rgba(255,237,95,1) 2),rgba(168,255,95,1) 3),rgba(131,255,247,1) 4),rgba(120,148,255,1) 5),rgb(216,117,255) 6),rgb(255,119,115) 7) ) 0% var(--bg-y)/200% 700% no-repeat",
    "--diagonal":
      "repeating-linear-gradient( 128deg,#0e152e 0%,hsl(180,10%,60%) 3.8%,hsl(180,10%,60%) 4.5%,hsl(180,10%,60%) 5.2%,#0e152e 10%,#0e152e 12% ) var(--bg-x) var(--bg-y)/300% no-repeat",
    "--shade":
      "radial-gradient( farthest-corner circle at var(--m-x) var(--m-y),rgba(255,255,255,0.1) 12%,rgba(255,255,255,0.15) 20%,rgba(255,255,255,0.25) 120% ) var(--bg-x) var(--bg-y)/300% no-repeat",
    backgroundBlendMode: "hue, overlay",
  };
  ...
  return (
    ...
    >
      <div className="grid h-full origin-center [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] overflow-hidden rounded-[var(--radius)] border border-oklch(0.922 0 0) border-slate-800 transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6] dark:border-oklch(1 0 0 / 10%)">
